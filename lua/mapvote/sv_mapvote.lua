...
util.AddNetworkString("RAM_RevoteRequest")
...
hook.Add("PlayerSay", "RevoteCommand", function(ply, text, teamChat)
    cmdStart, cmdEnd = string.find(text:lower(), "!revote")
    if cmdStart then
        net.Start("RAM_RevoteRequest")
        net.Send(ply)
    end
end)